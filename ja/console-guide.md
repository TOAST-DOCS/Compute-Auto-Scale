...
## Compute > Auto Scale > コンソール使用ガイド { #compute-auto-scale-console-guide }

<a id="instance-templates"></a>
## インスタンステンプレート { #instance-templates }
...
## スケーリンググループ { #scaling-groups }

<a id="view-list-of-scaling-groups"></a>
### スケーリンググループリスト表示 { #view-list-of-scaling-groups }
...
### スケーリンググループ生成 { #create-scaling-groups }
...
### ロードバランサー変更 { #change-load-balancer }
...
### スケーリンググループ停止 { #stop-scaling-group }
...
### スケーリンググループオーナーの変更 { #change-scaling-group-owner }
...
### 詳細情報表示、修正 { #view-details-and-modify }
スケーリンググループリストから、スケーリンググループを選択して詳細情報を確認します。

詳細情報画面で`変更`を選択すると、スケーリンググループの最小、最大、起動インスタンス数を変更できます。

<a id="view-policy-and-execute"></a>
### ポリシー表示、実行 { #view-policy-and-execute }
...
### 予約作業表示、生成 { #view-and-create-scheduled-tasks }
...
| 基準時間 | 予約作業の開始時間および終了時間のタイムゾーンを指定します |
| 開始時間 | 予約作業が有効化される時間<br>`繰り返し`を1回に選択した場合、開始時間に予約作業実行<br>`繰り返し`をCron式に選択した場合、開始時間を起点にして周期的に予約作業実行 |
...
### 生成したインスタンスリスト表示 { #view-list-of-created-instances }
...
### 統計グラフ表示 { #view-statistical-graphs }
